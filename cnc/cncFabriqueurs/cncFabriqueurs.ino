...
#include <AFMotor.h>			//Librairie pour piloter les moteurs pas à pas
#include <Servo.h>
...
const int penServoPin = 7;			// Pin sur lequel est relié le servomoteur
...
AF_Stepper myStepperY(stepsPerRevolution, 2);	// Axe X         
AF_Stepper myStepperX(stepsPerRevolution, 1);  // Axe Y

// Calibration, nombre de pas par millimètre
float StepsPerMillimeterX = 3.57;
float StepsPerMillimeterY = 1.785;
...
void stepX(int dist) {
  ...
  if (dist>0) {
    myStepperX.step(dist, FORWARD, MICROSTEP);	
  } 
  else {
    myStepperX.step(dist, BACKWARD, MICROSTEP);	
  }    
}
...
void stepY(int dist) {
  ...
  if (dist>0) {
    myStepperY.step(dist, FORWARD, MICROSTEP);	
  } 
  else {
    myStepperY.step(dist, BACKWARD, MICROSTEP);	
  }    
}
/**********************
...
void setup() {
  ...
  myStepperX.step(20, FORWARD, SINGLE); 
  myStepperX.step(20, BACKWARD, SINGLE); 
  myStepperY.step(20, FORWARD, SINGLE); 
  myStepperY.step(20, BACKWARD, SINGLE); 
